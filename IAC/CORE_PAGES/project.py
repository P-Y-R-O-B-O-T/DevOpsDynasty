...
class PROJECT:
    ...
    def load_modules_versions(self) -> None:
        ...
        for _ in os.listdir(os.path.join(CONSTANTS.MODULES_DIR)):
            m = importlib.import_module(
                f"{CONSTANTS.MODULES_DIR}.{_}.{CONSTANTS.VERSIONS_MODULE_FILE[:-3]}"
            )
    ...
    def list_projects(self):
        ...
        for _ in dirs:
            if not (
                ...
                in os.listdir(os.path.join(CONSTANTS.PROJECTS_DIR, _))
                and CONSTANTS.CONF_FILE
                in os.listdir(
                    os.path.join(CONSTANTS.PROJECTS_DIR, _, CONSTANTS.PROJ_CONF_DIR)
                )
                and CONSTANTS.RESOURCE_CONF_FILE
                in os.listdir(
                    os.path.join(CONSTANTS.PROJECTS_DIR, _, CONSTANTS.PROJ_CONF_DIR)
                )
            ):
            ...
            with open(
                ...
                st.session_state[CONSTANTS.EXISTING_PROJECTS][_] = {
                    CONSTANTS.CONF_EXISTING_PROJECTS: json.load(conf_file)
                }
        ...
        for _ in sorted(list(st.session_state[CONSTANTS.EXISTING_PROJECTS].keys())):
            with cols[count % len(cols)]:
                with st.container(border=True):
                    ...
                    st.write(
                        st.session_state[CONSTANTS.EXISTING_PROJECTS][_][
                            CONSTANTS.CONF_EXISTING_PROJECTS
                        ]
                    )
                    ...
                    with col1:
                        ...
                        if select_button:
                            ...
                            with open(
                                ...
                                st.session_state[CONSTANTS.RESOURCE_CONF] = json.load(
                                    resource_conf_file
                                )
    ...
    def delete_prompt(self, project: str) -> None:
        st.markdown(
            f"""
        ...
        prompt = st.text_input("", "", key=f"project_delete_prompt_{project}")
        ...
        if prompt == project and delete_button:
            try:
                os.system(
                    f"rm {os.path.join(CONSTANTS.PROJECTS_DIR,
                                             project)} -r"
                )
    ...
    def create_project(self) -> None:
        project_name = st.text_input(
            "Project Name", max_chars=20, key=f"create_project_projectname"
        )
        ...
        for _ in st.session_state[CONSTANTS.MODULES_VERSIONS]:
            select_boxes_providers[_] = st.selectbox(
                _.upper(),
        ...
        if submit_button and project_name:
            providers = {
                _: select_boxes_providers[_]
                for _ in select_boxes_providers
                if select_boxes_providers[_] is not None
            }
            ...
            os.mkdir(os.path.join(CONSTANTS.PROJECTS_DIR, project_name))
            os.mkdir(
                os.path.join(
                    CONSTANTS.PROJECTS_DIR, project_name, CONSTANTS.PROJ_CONF_DIR
                )
            )
            ...
            with open(
                ...
                "w",
            ) as provider_conf_file:
                json.dump(project_conf_base_format, provider_conf_file, indent=4)
            with open(
                os.path.join(
                    CONSTANTS.PROJECTS_DIR,
                    project_name,
            ...
            st.session_state[CONSTANTS.EXISTING_PROJECTS][project_name] = {
                CONSTANTS.CONF_EXISTING_PROJECTS: project_conf_base_format
            }
        ...
        if CONSTANTS.SELECTED_PROJECT not in st.session_state:
            ...
            if os.path.exists(
                os.path.join(
                    ...
                    f"{st.session_state[CONSTANTS.SELECTED_PROJECT]}.{CONSTANTS.COMPRESSION_FORMAT}",
                )
            ):
                os.system(
                    f"rm {os.path.join(CONSTANTS.DOWNLOAD_DIR, st.session_state[CONSTANTS.SELECTED_PROJECT])}.{CONSTANTS.COMPRESSION_FORMAT}"
                )
            generete_file = st.button(
                "Generete File",
                use_container_width=True,
                key=f"generate_download_file_{st.session_state[CONSTANTS.SELECTED_PROJECT]}",
            )
            if generete_file:
                UD.gen_zip_file()
            if os.path.exists(
    ...
    def upload_project(self) -> None:
        ...
        file_upload = st.file_uploader(
            f"Upload a project file (.{CONSTANTS.COMPRESSION_FORMAT})",
        ...
        if file_upload is not None:
            ...
            if file_upload.name[-7:] != f".{CONSTANTS.COMPRESSION_FORMAT}":
                TOAST.create_toast(
                    f"Upload a tar file with xz compression only (.{CONSTANTS.COMPRESSION_FORMAT})",
                    "⚠️",
                )
                st.rerun()
            ...
            try:
                with open(
                    os.path.join(CONSTANTS.UPLOAD_DIR, file_upload.name), "wb"
                ) as upload_file:
            ...
            try:
                os.system(
                    f"tar --extract --file {os.path.join(CONSTANTS.UPLOAD_DIR,
                                                               file_upload.name)} -C {CONSTANTS.UPLOAD_DIR}"
                )
                os.system(
                    f"rm {os.path.join(CONSTANTS.UPLOAD_DIR,
                                             file_upload.name)}"
                )
            except:
            ...
            os.system(
                f"mv {os.path.join(CONSTANTS.UPLOAD_DIR,
