        ...
        if submit_button:
            if name and policy:
                if not IV.a_zA_z0_9(name):
                    TOAST.create_toast(
                        "Role name can only have [A-Za-z0-9] and length less then 53",
                        "⚠️",
                    )
                    st.rerun()
                self.add_resource(name=name, data={"name": name, "policy": policy})
    ...
    def modify_resource_5_81_0(self, name: str, data: dict) -> None:
        ...
        if submit_button:
            if name and policy:
                ...
                self.add_resource(
                    name=name, data={"name": name, "policy": policy}, force=True
                )
